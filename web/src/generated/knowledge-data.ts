...
export const knowledgeDocs: KnowledgeDoc[] = [
  ...
  },
  {
    ...
    "headings": [
      ...
      {
        "level": 3,
        "text": "6. 合并 PR 后 Issue 如何解决",
        "slug": "6-合并-pr-后-issue-如何解决"
      },
      {
        "level": 3,
        "text": "7. 信息不足时如何处理",
      ...
      {
        "level": 3,
        "text": "9. 为什么不在 GitHub Actions 里跑 Codex",
        "slug": "9-为什么不在-github-actions-里跑-codex"
      },
      {
        "level": 2,
        "text": "关联",
...
export const knowledgeChunks: KnowledgeChunk[] = [
  ...
  },
  {
    ...
    "docId": "content/tech/tools/knowledge-base-workflow",
    "heading": "5. PR 创建后推送飞书通知",
    "text": "runner 创建 PR 后，会先在原 Issue 评论 PR 链接，再根据本地环境变量决定是否推送飞书通知。 飞书通知由服务器本地配置控制： 如果 FEISHUWEBHOOK 为空，runner 只创建 PR 和评论 Issue，不发送飞书消息。如果 FEISHUWEBHOOK 有值，runner 会向飞书自定义机器人发送文本消息。消息不能只包含分支或 PR 链接，还要给 reviewer 足够上下文： 其中 Why 来自反馈 Issue 标题，说明为什么需要这次修改；What changed 来自 Codex 写入的结果摘要，说明这次实际改了什么。 配置步骤： 1. 在目标飞书群里添加「自定义机器人」。 2. 复制机器人 webhook URL。 3. 写入服务器本地环境文件 /.config/knowledge-agent/env。 4. 确认环境文件权限只允许当前用户读取。 示例： 安全设置建议： - 优先开启飞书机器人的关键词校验，关键词可以设置为 Knowledge feedback PR created。 - 如果开启签名校验，需要给 runner 增加 timestamp 和 sign 计算逻辑；当前脚本只支持普通 webhook。 - 如果开启 IP 白名单，需要加入运行 agent 的服务器出口 IP。 - webhook 属于敏感凭据，只保存在服务器本地环境文件，不提交到仓库。 验证方式： 只有当本轮确实处理了 content-feedback Issue 并成功创建 PR 时，飞书才会收到通知。没有可处理 Issue、信息不足、验证失败或 PR 创建失败，都不会发送飞书消息。",
    "searchText": "知识库使用与反馈流程 5. pr 创建后推送飞书通知 runner 创建 pr 后，会先在原 issue 评论 pr 链接，再根据本地环境变量决定是否推送飞书通知。 飞书通知由服务器本地配置控制： 如果 feishuwebhook 为空，runner 只创建 pr 和评论 issue，不发送飞书消息。如果 feishuwebhook 有值，runner 会向飞书自定义机器人发送文本消息。消息不能只包含分支或 pr 链接，还要给 reviewer 足够上下文： 其中 why 来自反馈 issue 标题，说明为什么需要这次修改；what changed 来自 codex 写入的结果摘要，说明这次实际改了什么。 配置步骤： 1. 在目标飞书群里添加「自定义机器人」。 2. 复制机器人 webhook url。 3. 写入服务器本地环境文件 /.config/knowledge-agent/env。 4. 确认环境文件权限只允许当前用户读取。 示例： 安全设置建议： - 优先开启飞书机器人的关键词校验，关键词可以设置为 knowledge feedback pr created。 - 如果开启签名校验，需要给 runner 增加 timestamp 和 sign 计算逻辑；当前脚本只支持普通 webhook。 - 如果开启 ip 白名单，需要加入运行 agent 的服务器出口 ip。 - webhook 属于敏感凭据，只保存在服务器本地环境文件，不提交到仓库。 验证方式： 只有当本轮确实处理了 content-feedback issue 并成功创建 pr 时，飞书才会收到通知。没有可处理 issue、信息不足、验证失败或 pr 创建失败，都不会发送飞书消息。"
  },
  {
    "id": "content/tech/tools/knowledge-base-workflow::18",
    "docId": "content/tech/tools/knowledge-base-workflow",
    "heading": "6. 合并 PR 后 Issue 如何解决",
    ...
    "searchText": "知识库使用与反馈流程 6. 合并 pr 后 issue 如何解决 issue 不由 agent 直接关闭，而是在 pr 合并后由 github 自动关闭。 例如： 当 pr #6 合并到 main 后： - github 自动关闭 issue #5。 - github actions 按主分支更新触发构建发布。 - 下一轮 agent 只扫描 open issue，所以不会再处理 #5。 如果 pr 被关闭但没有合并，issue 仍然 open；这时后续定时扫描可能会再次处理这个 issue。"
  },
  {
    ...
    "searchText": "知识库使用与反馈流程 7. 信息不足时如何处理 如果 ai 判断 issue 信息不足，会返回 blocked。 runner 会： - 给 issue 加上 content-feedback-blocked 标签。 - 评论说明缺少什么信息。 - 跳过这个 issue，直到有人补充信息。 处理方式： 1. 直接编辑同一个 issue。 2. 补充清楚 问题说明 和 期望修改。 3. 移除 content-feedback-blocked 标签。 4. 等下一轮定时扫描重新处理。"
  },
  {
    "id": "content/tech/tools/knowledge-base-workflow::20",
    "docId": "content/tech/tools/knowledge-base-workflow",
    "heading": "8. 当前服务器配置",
    "text": "运行配置在服务器本地，不提交到仓库： 核心配置项： FEISHUWEBHOOK 可以留空。留空表示关闭飞书通知；配置后表示 PR 创建成功时通知飞书群。 systemd 配置： 常用操作： 临时 worktree 不包含被 Git 忽略的 web/nodemodules。runner 会把主仓库的 web/nodemodules 链接到临时 worktree 中，所以服务器主仓库需要先安装前端依赖：",
    "searchText": "知识库使用与反馈流程 8. 当前服务器配置 运行配置在服务器本地，不提交到仓库： 核心配置项： feishuwebhook 可以留空。留空表示关闭飞书通知；配置后表示 pr 创建成功时通知飞书群。 systemd 配置： 常用操作： 临时 worktree 不包含被 git 忽略的 web/nodemodules。runner 会把主仓库的 web/nodemodules 链接到临时 worktree 中，所以服务器主仓库需要先安装前端依赖："
  },
  {
    "id": "content/tech/tools/knowledge-base-workflow::21",
    "docId": "content/tech/tools/knowledge-base-workflow",
    "heading": "9. 为什么不在 GitHub Actions 里跑 Codex",
    ...
    "searchText": "知识库使用与反馈流程 9. 为什么不在 github actions 里跑 codex 当前选择个人服务器定期扫描，而不是 github actions 直接跑 codex，主要原因是： - codex 登录态和个人配置留在服务器本地，避免提交到仓库或 ci 配置中。 - runner 可以使用本地已登录的 gh 和 codex。 - 可以以后替换 agentprovider，例如从 codex 换成 claude。 - github actions 只负责合并后的构建发布，避免每个反馈分支都自动打包镜像。 这个设计的关键边界是：ai 负责提出修改，人工通过 pr review 决定是否合并。"
  },
  {
    "id": "content/tech/tools/knowledge-base-workflow::22",
    "docId": "content/tech/tools/knowledge-base-workflow",
    "heading": "关联",
